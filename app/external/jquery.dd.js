...
(function (factory) {
    ...
    function Dd(element, settings) {
        ...
        var createByJson = function () {
            if (settings.byJson.data) {
                ...
                try {
                    ...
                    settings.byJson.data = JSON.parse(settings.byJson.data);
                    //change element
                    var id = "msdropdown" + (msBeautify.counter += 1);
                    var obj = {};
                    obj.id = id;
                    ...
                    var i = 0;
                    var current, opt, p, key;
                    for (i = 0; i < settings.byJson.data.length; i += 1) {
                        current = settings.byJson.data[i];
                        opt = new Option(current.text, current.value);
                        for (p in current) {
                            if (p.toLowerCase() !== 'text') {
                                key = ($.inArray(p.toLowerCase(), validData) !== -1) ? "data-" : "";
        ...
        var init = function () {
            ...
            createByJson();
            if (!element.id) {
                element.id = "msdrpdd" + (msBeautify.counter += 1);
            }
            element = element.id;
        ...
        var parseOption = function (opt) {
            ...
            if (opt !== undefined) {
                ...
                if (attrTitle !== "") {
                    ...
                    var obj = (isJson && settings.jsonTitle) ? JSON.parse("[" + attrTitle + "]") : null;
        ...
        var createChild = function (opt) {
            ...
            if (parsed.description !== "") {
                oDescription = createElement("span", {
                    className: css.description
                }, parsed.description);
            }
            var sText = opt.text || "";
        ...
        var createChildren = function () {
            ...
            var allOptions = getElement(element).children,
                i, current, li, span, optChildren, optul, j, opt_li;
            for (i = 0; i < allOptions.length; i += 1) {
                current = allOptions[i];
                if (current.nodeName.toLowerCase() === "optgroup") {
                    ...
                    li.appendChild(span);
                    optChildren = current.children;
                    optul = createElement("ul");
                    for (j = 0; j < optChildren.length; j += 1) {
                        opt_li = createChild(optChildren[j]);
                    ...
                    if (isMultiple) {
                        if (shiftHolded === true) {
                            ...
                            if (selected.length > 1) {
                                ...
                                for (var i = Math.min(ind1, ind2); i <= Math.max(ind1, ind2); i += 1) {
                                    var current = items[i];
